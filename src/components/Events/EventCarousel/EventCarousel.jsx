...
const EventCarousel = () => {
    ...
    const nextSlide = useCallback(() => {
        ...
        newToShow[(current - 1 + length) % length] = (newToShow[(current + 1) % length] + 1) % contentLength;
        setToShow(newToShow);
    ...
    const prevSlide = useCallback(() => {
        ...
        //Change the toshow array to change content of carousel
        let newToShow = [...toshow];
        newToShow[(current + 1) % length] = (newToShow[(current - 1 + length) % length] - 1 + contentLength) % contentLength;
        setToShow(newToShow);
        ...
        <>
            <div className='EventCarousel_mainContainer'>
                    ...
                    <div className="CardCarousel_container">
                        {
                            helperArray.map((element, idx) => {
                                return (
                                    <div key={idx} className={`EventCarousel_card
                                        ${
                                        ...
                                        {Events[toshow[idx]].description.Heading}
                                    </div>
                                )
                            })
                        }
                ...
                <div className='EventCarousel_SlideButton'>
                    {Events.map((ele, idx) => {
                        return (
                            <div key={idx} className={`SlideBtn${toshow[current] === idx ? " active" : ""}`} onClick={() => SlideBtnClickHandler(idx)} />
                        )
                    })}
